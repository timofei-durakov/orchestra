...
trait ConfigYamlProtocol extends DefaultYamlProtocol {
  ...
  implicit val scenarioFormat = yamlFormat7(Scenario)
...
final case class Scenario(id: Int, parallel: Int, playbook_path: String, hosts: List[String], on_finish: List[String], on_sync_events: List[String], steps: List[String])
