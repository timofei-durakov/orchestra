...
trait ConfigYamlProtocol extends DefaultYamlProtocol {
  ...
  implicit val scenarioFormat = yamlFormat5(Scenario)
...
final case class Scenario(id: Int, parallel: Int, on_finish:List[String], on_sync_events:List[String], steps:List[String])
